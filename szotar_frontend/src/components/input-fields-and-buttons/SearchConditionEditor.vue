        ...
        @click="() => {
          if (modelValue.length) {
            $emit(`update:modelValue`,
              modelValue.length === 1 ?
              modelValue[0].expression.split(` `).map(expression=>({
                onlyWithSpaceDotOrCommaSuffix: modelValue[0].onlyWithSpaceDotOrCommaSuffix,
                onlyWithSpacePrefix: modelValue[0].onlyWithSpacePrefix,
                expression
              })):
              [{
                onlyWithSpaceDotOrCommaSuffix: modelValue[0].onlyWithSpaceDotOrCommaSuffix,
                onlyWithSpacePrefix: modelValue[0].onlyWithSpacePrefix,
                expression: modelValue.map(e=>e.expression).join(' '),
              }] 
            )
          }}"
        >Egybe / külön</button>
