...
const dudenEntryUrlFormat = (word: string) => {
...
const encodeURI = (str: string, encoding?: string) => {
...
const generateLinks = (lang: string, phrase: string) => {
  const all: Record<string,GeneratedLink[]> = {
    'de': [
      {
        title: `Wiktionary`,
...
router.get('/generate_links_for_entry', async (req, res) => {
  const reqParams = req.query as {lang?: string, phrase?: string}
  const lang: string = reqParams?.lang ?? ``;
  if (lang.trim()===``) {res.status(400).send(`"lang" param must not be empty`); return;}
  const phrase: string = reqParams?.phrase?.trim() ?? ``;
  if (phrase.trim()===``) {res.status(400).send(`"phrase" param must not be empty`); return;}
